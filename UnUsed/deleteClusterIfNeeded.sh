...
LOADBALANCEDCLUSTER_EXIST=$(kubectl config get-contexts -o name | grep loadbalancedcluster | wc -c)
echo "Do clusters exist?-" $LOADBALANCEDCLUSTER_EXIST
if [[ LOADBALANCEDCLUSTER_EXIST -ne 0 ]]; then
	LOADBALANCEDCLUSTER=`kubectl config get-contexts -o name | grep loadbalancedcluster`
